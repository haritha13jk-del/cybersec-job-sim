...
router.post('/chat', authMiddleware, async (req, res) => {
  try {
    ...
    const aiResponse = await generateAIResponse(message, scenario);
    const query = `INSERT INTO chat_history (user_id, scenario_id, message, response) VALUES (?, ?, ?, ?)`;
    await mysqlPool.query(query, [userId, scenarioId || null, message, aiResponse.message]);
...
router.post('/hint', authMiddleware, async (req, res) => {
  try {
    ...
    const [hintCount] = await mysqlPool.query(
      'SELECT COUNT(*) as count FROM chat_history WHERE user_id = ? AND scenario_id = ? AND message LIKE ?',
      [userId, scenarioId, '%hint%']
    );
    const attemptNumber = hintCount[0].count + 1;
    let hint;
    ...
    await mysqlPool.query(
      'INSERT INTO chat_history (user_id, scenario_id, message, response) VALUES (?, ?, ?, ?)',
...
router.get('/history', authMiddleware, async (req, res) => {
  try {
    ...
    let query = `SELECT ch.*, s.title as scenario_title FROM chat_history ch LEFT JOIN scenarios s ON ch.scenario_id = s.id WHERE ch.user_id = ?`;
    const params = [userId];
    if (scenarioId) {
